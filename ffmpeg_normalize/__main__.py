...
extension = ".wav"
def get_all_files(dir_list=[], extension=extension):
    input_files = []
    output_files = []
    if dir_list:
        for walk_dir in dir_list:
            w = walk_dir
            ...
            for root, subdirs, files in os.walk(walk_dir):
                ...
                for filename in files:
                    if extension in filename:
                        ...
                        out_path = root.replace(w, "./{0}-{1}".format(w,"norm"))
                        if not os.path.exists(out_path):
                            os.makedirs(out_path)
                        # else:
                        #     print("Directory " , dirName ,  " already exists")
                        o_file = os.path.join(out_path, filename)
                        output_files.append(o_file)
...
def main():
    ...
    print("INPUT DIR'S ARE :: ", cli_args.input)
    
    input_files_list, output_file_list = get_all_files(dir_list=cli_args.input)
    #quit()
    ...
    for index, input_file in enumerate(input_files_list):
        ...
        if cli_args.output is not None and index < len(cli_args.output):
            ...
            # basepath = input_file.replace(basename, "")
            # basename = basename.split(".")
            output_file = output_file_list[index]
            # output_file = os.path.join(
            #     basepath,
            #     "{0}_{1}".format(basename[0],"normalized") +
            #     '.' + basename[1],
            # )
            # output_file = os.path.join(
